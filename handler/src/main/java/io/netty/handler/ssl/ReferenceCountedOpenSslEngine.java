...
public class ReferenceCountedOpenSslEngine extends SSLEngine implements ReferenceCounted {
    ...
    private SSLEngineResult readPendingBytesFromBIO(ByteBuffer dst, int consumed, int produced, int pendingNet,
        ...
        // It's important we call this before wrapStatus() as wrapStatus() may also shutdown the engine.
        SSLEngineResult.HandshakeStatus hs = mayFinishHandshake(
                status != FINISHED ? getHandshakeStatus(pendingNet - readBytes) : status);

        return new SSLEngineResult(wrapStatus(), hs, consumed, produced + readBytes);
    }
                    ...
                    if (result > 0) {
                        ...
                        switch (sslError) {
                            ...
                            case SSL.SSL_ERROR_WANT_READ:
                                ...
                                if (pendingNet <= 0) {
                                    return new SSLEngineResult(wrapStatus(), NEED_UNWRAP, bytesConsumed, bytesProduced);
                                }
                                break;
    ...
    private SSLEngineResult.Status wrapStatus() {
